...
We use [`cargo-release`](https://github.com/crate-ci/cargo-release) to make releases, which you can install with `cargo binstall cargo-release` or `cargo install cargo-release`. To update the CI scripts, run `dist generate-ci`, which uses [cargo-dist](https://github.com/axodotdev/cargo-dist).
